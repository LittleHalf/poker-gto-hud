...
export async function handIngest(event: GameEvent, session_id: string) {
  await insertHandEvent(session_id, event)
  ...
  if (event.type === 'PLAYER_JOIN' || event.type === 'HAND_START') {
    ...
    for (const p of players) {
      await upsertPlayer(p.id, p.name)
    }
  }
  ...
  if (event.type === 'ACTION') {
    ...
    if (player_id && action) {
      await updateStats(player_id, action, handState)
    }
  }
  ...
  if (event.type === 'SHOWDOWN') {
    ...
    for (const p of players) {
      await updateStats(p.id, 'SHOWDOWN', handState)
    }
  }
