...
const LLM_NOTES_THRESHOLD = 10  // showdowns before generating notes
...
export async function dbLookup(player_id: string): Promise<PlayerProfile> {
  ...
  const rawStats = getStats(player_id)
  const tag = rawStats ? computeTag(rawStats) : 'UNKNOWN'

  const sample = rawStats?.vpip_denom ?? 0
  const confidence: PlayerProfile['confidence'] =
    sample < 5 ? 'low' : sample < 30 ? 'medium' : 'high'
  ...
  // Generate LLM notes if threshold met and notes not yet generated
  ...
  if (!llm_notes && (rawStats?.wtsd_denom ?? 0) >= LLM_NOTES_THRESHOLD) {
    ...
    db.prepare('UPDATE players SET llm_notes = ? WHERE id = ?').run(llm_notes, player_id)
  }
...
async function generateLlmNotes(
  ...
  const prompt = `You are a poker analyst. Summarize this player's tendencies in 1-2 concise sentences for a HUD tooltip.

Player: ${name}
Stats (${stats.sample_size} hands):
- VPIP: ${stats.vpip !== null ? (stats.vpip * 100).toFixed(1) + '%' : 'N/A'}
- PFR: ${stats.pfr !== null ? (stats.pfr * 100).toFixed(1) + '%' : 'N/A'}
- AF (Aggression Factor): ${stats.af !== null ? stats.af.toFixed(2) : 'N/A'}
- Fold to C-bet: ${stats.fold_to_cbet !== null ? (stats.fold_to_cbet * 100).toFixed(1) + '%' : 'N/A'}
- Fold to 3-bet: ${stats.fold_to_3bet !== null ? (stats.fold_to_3bet * 100).toFixed(1) + '%' : 'N/A'}
- WTSD (Went to Showdown): ${stats.wtsd !== null ? (stats.wtsd * 100).toFixed(1) + '%' : 'N/A'}
